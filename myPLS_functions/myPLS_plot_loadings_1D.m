...
nBootstrap= size(vars_b_vect,3);
nLCs = size(vars_b_vect,2);

nBehav = nVars/nGroups;

% setup plotting colors depending on var_type
...
% reorder, to group together the same behavior variables of different groups
iter=1;
for iB=1:nBehav
...
for iter_lc = 1:length(signif_LC)
    ...
    % scatterplot, if asked for
    if save_opts.plot_boot_samples
        rand_x = [1:nVars]+0.08*randn(nBootstrap,1); % helper variable for scatterplots
...
%         s=scatter(rand_x(:),vars_tmp(:),12,plot_col,'filled','MarkerFaceAlpha',.5);
        s=scatter(rand_x(:),vars_tmp(:),12,repmat(repelem(plot_col,nBootstrap,1),nBehav,1),...
